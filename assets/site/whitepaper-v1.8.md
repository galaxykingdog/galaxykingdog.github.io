...
> **Historical Solana v1.8 document.** Its technical sections are preserved below. The current Robinhood reward candidate is a separate, undeployed project phase.
...
Next: independent contract and replay review, a real hosting rehearsal with Android and wallet checks, verified administrator custody and recipient/token addresses, and NFT publication. The public arcade and historical testnet remain separate from this candidate.

## Historical Solana v1.8 reference

Historical Solana v1.8 implementation notes, retained for reference. For the frozen numeric constants see
[WHITEPAPER_IMPLEMENTATION_LOCK_V1_7.md](papers/whitepaper_implementation_lock_v1_7.md); per-subsystem
...
See [ANTI_CHEAT_SCORE.md](papers/anti_cheat_score.md). A fully independent, bit-for-bit deterministic replay
...
pool funds** — there is no such instruction. See [FEE_GOVERNANCE.md](papers/fee_governance.md).
...
share recycles back** into the Game Pool. See [POOL_MECHANISM.md](papers/pool_mechanism.md).
...
[TOKEN_MINING.md](papers/token_mining.md).
...
number blocks duplicates — and every NFT is minted by the program PDA. See [NFT_AWARDS.md](papers/nft_awards.md).
...
whitelisted, verified charity operator. See [CHARITY_GOVERNANCE.md](papers/charity_governance.md).
...
- Known centralization to harden before mainnet: the **admin authority** is a single key (it cannot
  ...
  **verifier key** is a trusted oracle. Recommended: multisig admin, HSM-held verifier key, a
  governance balance snapshot, and an **independent audit**. See [SECURITY_NOTES.md](papers/security_notes.md).
...
deployment and the operational hardening above. See [DEPLOY.md](papers/deploy.md).
